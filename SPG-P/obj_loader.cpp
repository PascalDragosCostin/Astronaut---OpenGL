...
void CalcNormals(IndexedModel &indexModel)
{
    for (unsigned int i = 0; i < indexModel.indices.size(); i += 3)
    {
        int i0 = indexModel.indices[i];
        int i1 = indexModel.indices[i + 1];
        int i2 = indexModel.indices[i + 2];

        glm::vec3 v1 = indexModel.positions[i1] - indexModel.positions[i0];
        glm::vec3 v2 = indexModel.positions[i2] - indexModel.positions[i0];

        glm::vec3 normal = glm::normalize(glm::cross(v1, v2));

        indexModel.normals[i0] += normal;
        indexModel.normals[i1] += normal;
        indexModel.normals[i2] += normal;
    }

    for (unsigned int i = 0; i < indexModel.positions.size(); i++)
        indexModel.normals[i] = glm::normalize(indexModel.normals[i]);
};
